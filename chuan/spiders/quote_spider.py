...
class FahasaSpider(scrapy.Spider):
    ...
    start_urls = [
      ...
      "https://www.youtube.com/watch?v=onQLjY3Nosk"
    ]
    ...
    def start_requests(self):
        for url in self.start_urls:
            yield SplashRequest(url, endpoint='execute',
                                args={'wait': 10,'lua_source': script3, 'timeout':3600, 'nums': 5}, callback = self.parse )
    ...
    def parse(self, response):
        ...
        for xtext in comment: 
            ...
            print("\n\n%s"%commenttex.select("text()").extract_first()) 
            ...
            print("\nLikecommrnt: %s"%like.select("text()").extract_first())
            j = 0
            ...
            for text in replie: 
                ...
                print("\nreplie : %s"%text.select("text()").extract_first()) 
                j= j+1
                listreply.append(itemReplies)
            itemcomment["Replies"] = listreply
            itemcomment["CountReplies"] = j+1
            listcomment.append(itemcomment)
            i = i+1
        item["CountComment"] = i
        item["Comment"] = listcomment
        print("\n\n\n\n number comment: ",i)
        print("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nITEM\n")
        print(item)
